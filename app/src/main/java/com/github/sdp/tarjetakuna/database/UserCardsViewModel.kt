...
class UserCardsViewModel : ViewModel() {
    ...
    fun onSetButtonClick(card: MagicCard) {
        testCollection.forEach(usc::addCardToCollection)
        _setMessage.value = Pair(R.string.text_add_success, "Cards")
    ...
    fun onRemoveButtonClick(card: MagicCard) {
        testCollection.forEach(usc::removeCardFromCollection)
        _removeMessage.value = Pair(R.string.text_remove_success, "Cards")
    }
    ...
    fun onGetButtonClick(card: MagicCard) {
        val data = usc.getCardFromCollection(testCollection.get(0))
        data
            .thenAccept {
